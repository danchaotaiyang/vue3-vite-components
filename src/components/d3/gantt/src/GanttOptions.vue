...
export default {
    ...
    computed: {
        tipStyle() {
            let { width, tipPositionRef, offsetX } = this;
            let [ x, y ] = tipPositionRef;
            x += offsetX;
            if (this.$refs.tipRef) {
                const { clientWidth } = this.$refs.tipRef;
    ...
    methods: {
        getInterpolate(item, field, value) {
        ...
        setTips(info) {
            if (info) {
                ...
                value = `${ value * 100 }%`;
                this.tipInfoRef = { name, start, end, value, bgColor, color };
            } else {
                this.tipInfoRef = null;
            }
        },
        init() {
            let { width, height, mainRange, crossRange, atLast } = this;
        ...
        create() {
            let { data, width, height, itemHeight, offsetY, offsetStart, offsetEnd, atLast } = this;
        ...
        setData() {
            let { data, itemHeight, offsetY, offsetStart, offsetEnd, duration, ease, color } = this;
            ...
            $tasksValue
                ...
                .attrTween('fill', (d, i, a) => {
                    ...
                    return this.getInterpolate(a[ i ], 'currentFill', color);
                })
            ...
            /*$tasksValue
                ...
                .attrTween('transform', (d, i, a) => {
                    ...
                    let current = a[ i ][ 'currentTranslate' ];
                    
                    if (typeof current === 'undefined') {
                        current = 0;
                    }
                    
                    let interpolate = d3[ 'interpolate' ](current, __scale__time__(d.start));
                    
                    return t => {
                        a[ i ][ 'currentTranslate' ] = interpolate(t);
                        return `translate(${ a[ i ][ 'currentTranslate' ] }, ${ i * __step__height__ + offsetY })`;
                    };
                })
                .attrTween('fill', (d, i, a) => {
                ...
                .attrTween('d', (d, i, a) => {
                    ...
                    return t => {
                        a[ i ][ 'currentWidth' ] = interpolate(t);
                        return `M${ a[ i ][ 'currentWidth' ] },${ itemHeight } H4c-2.21,0-4-1.79-4-4V4C0,1.79,1.79,0,4,0H${ a[ i ][ 'currentWidth' ] }s0,${ itemHeight },0,${ itemHeight }Z`;
                    };
                });*/
            ...
            $panel
                .on('mousemove', (e) => {
                    ...
                    if (data[ y ] && x >= data[ y ][ 'start' ] && x <= data[ y ][ 'end' ]) {
                        this.tipPositionRef = [ mouseX, mouseY ];
                        this.setTips(data[ y ]);
                    } else {
                        this.setTips();
                    }
    ...
    watch: {
        data(nv) {
            if (Array.isArray(nv) && nv.length) {
                typeof $svg === 'undefined' && this.create();
                this.setData();
            }
        }
    }
...
<template>
<div class="d3-gantt-vue-options">
