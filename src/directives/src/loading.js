...
export default {
    ...
    mounted(el, binding) {
        if (binding.value) {
            el._loading = ElLoading.service({ ...option, target: el });
        }
    },
    updated(el, binding) {
        if (binding.value && !el._loading) {
            el._loading = ElLoading.service({ ...option, target: el });
        } else if (!binding.value && el._loading) {
            el._loading.close();
            el._loading = null;
        }
    },
