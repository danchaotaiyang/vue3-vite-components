...
const { BMAP_ANCHOR_BOTTOM_LEFT } = window;
...
const markers = defineModel('markers', {
    default: () => []
    // 结构示例:
    /*
    [
...
const polylines = defineModel('polylines', {
    default: () => []
    // 结构示例:
    /*
    [
...
const polygons = defineModel('polygons', {
    default: () => []
    // 结构示例:
    /*
    [
...
const circles = defineModel('circles', {
    default: () => []
    // 结构示例:
    /*
    [
...
const customStyle = defineModel('customStyle', {
    default: null
});
...
// 添加日志工具函数
const logger = {
    log: (message, ...args) => props.debug ? console.log(`【百度地图组件】${ message }`, ...args) : null,
    warn: (message, ...args) => {
...
const loadBaiduMapAPI = () => {
    // 检查是否正在加载或已加载
    if (window.__baiduMapLoadingPromise) {
    ...
    const loadingPromise = new Promise((resolve, reject) => {
        ...
        window._baiduMapCancelled = false;
        ...
        if (window.BMap) {
            ...
            emit('api-loaded', { BMap: window.BMap });
            resolve(window.BMap);
            return;
        }
        ...
        window[ callbackName ] = () => {
            ...
            const apiStatus = {
                BMap: !!window.BMap,
                BMAP_NORMAL_MAP: window.BMAP_NORMAL_MAP !== undefined,
                BMAP_SATELLITE_MAP: window.BMAP_SATELLITE_MAP !== undefined
            };
        ...
        if (!props.offline) {
            ...
            //加载地图API主文件
            script.src = window.mapConfig?.api;
            script.async = true;
    ...
    // 在全局存储加载 Promise
    window.__baiduMapLoadingPromise = loadingPromise;

    // 在 Promise 状态确定后从全局清除
    loadingPromise.finally(() => {
        // 添加一个短暂延迟，允许任何依赖的代码运行
        setTimeout(() => {
            delete window.__baiduMapLoadingPromise;
...
const initMap = async () => {
    ...
    try {
        ...
        // 处理地图类型设置
        try {
            if (!props.offline && window[ props.type ]) {
                // 如果存在指定的常量，使用它
                logger.log(`使用地图类型: ${ props.type }`);
                map.setMapType(window[ props.type ]);
            } else if (props.type === 'BMAP_NORMAL_MAP' && window.BMAP_NORMAL_MAP !== undefined) {
                // 普通地图类型
                logger.log('使用普通地图类型');
                map.setMapType(window.BMAP_NORMAL_MAP);
            } else if (props.type === 'BMAP_SATELLITE_MAP' && window.BMAP_SATELLITE_MAP !== undefined) {
                // 卫星地图类型
                logger.log('使用卫星地图类型');
...
const destroy = () => {
    ...
    window._baiduMapCancelled = true;
...
// 监听props变化
watch(() => props.center, debounce((nv, ov) => {
...
watch(() => props.zoom, debounce((nv, ov) => {
    // 确保nv是有效数字且与旧值不同
    if (map && typeof nv === 'number' && nv !== ov) {
        try {
            logger.log(`设置新缩放级别 (debounced): ${ nv }`);
            map.setZoom(nv);
        } catch (err) {
            logger.warn('设置缩放级别失败:', err);
        }
 ...
 */
const createGraphic = (type, data, creator) => {
    try {
        // 创建图形实例
...
const createPoint = (coords) => {
    if (coords && coords.length === 2) {
        try {
            return new BMap.Point(coords[ 0 ], coords[ 1 ]);
        } catch (err) {
 ...
 * @param {Array<number>} [labelData.offset] 标签文本相对于定位点的偏移
 * @param {number} [labelData.zIndex] 标签的zIndex
 * @returns {BMap.Label|null} 创建的标签对象或null
 */
const createLabel = (labelData) => {
    return createGraphic('label', labelData, (data) => {
        ...
        // 创建标签覆盖物
        let label = new BMap.Label(data.content, {
            offset: data.offset ? new BMap.Size(data.offset[ 0 ], data.offset[ 1 ]) : new BMap.Size(0, 0),
            position: point
        });
 ...
 */
function addGraphic(type, data, creator) {
    if (!map) return false;
    try {
        const graphic = creator(data);
        if (graphic) {
        ...
        logger.warn(`${ type }创建失败，未添加到地图:`, data);
        return false;
    } catch (err) {
 ...
 * 通用批量添加图形到地图和实例列表
 * @param {string} type 图形类型
 * @param {Array<Object>} list 图形数据列表
 * @param {Function} creator 图形创建函数
 */
function addGraphics(type, list, creator) {
    if (!map) return;
    if (type === 'trajectories') {
...
/**
 * 通用通过ID移除地图上的图形和实例列表中的对应项
 * @param {string} type 图形类型
 * @param {string} id 图形数据的唯一ID
 * @returns {boolean} 是否成功移除
 */
function removeGraphic(type, id) {
    if (!map) return false;
    try {
        if (type === 'trajectories') {
            // TrajectoryAnimation is a Map, not an array
            ...
            const arr = instances[ type ];
            const idx = arr.findIndex((g) => g.userData && g.userData.id === id);
            if (idx === -1) {
 ...
 */
function updateGraphic(type, id, updater) {
    if (!map) return false;
    try {
        if (type === 'trajectories') {
            ...
            const arr = instances[ type ];
            const graphic = arr.find((g) => g.userData && g.userData.id === id);
            if (!graphic) {
...
const addLabel = (labelData) => addGraphic('labels', labelData, createLabel);
const addLabels = (labelsList) => addGraphics('labels', labelsList, createLabel);
const removeLabel = (id) => removeGraphic('labels', id);
const clearLabels = () => clearGraphics('labels');
const updateLabel = (id, newContent, newStyle = null) => updateGraphic('labels', id, (label) => { // 参数 label 声明类型
    if (newContent) label.getContent().innerHTML = newContent; // 断言类型
    if (newStyle) Object.assign(label.getContent().style, newStyle); // 断言类型
});

const addMarker = (options = {}) => addGraphic('markers', options, createMarker);
const addMarkers = (markersList) => addGraphics('markers', markersList, createMarker);
const removeMarker = (id) => removeGraphic('markers', id);
const clearMarkers = () => clearGraphics('markers');
const updateMarker = (id, newPosition, newOptions = {}) => updateGraphic('markers', id, (marker) => {
    if (newPosition) {
    ...
    if (newOptions.icon) {
        ...
        const icon = new BMap.Icon(url, new BMap.Size(size[ 0 ], size[ 1 ]), {
            ...
            imageOffset: newOptions.icon.imageOffset ? new BMap.Size(imageOffset[ 0 ], imageOffset[ 1 ]) : null
        });
        marker.setIcon(icon);
    }
...
const addPolyline = (options = {}) => addGraphic('polylines', options, createPolyline);
const addPolylines = (polylinesList) => addGraphics('polylines', polylinesList, createPolyline);
const removePolyline = (id) => removeGraphic('polylines', id);
const clearPolylines = () => clearGraphics('polylines');
const updatePolyline = (id, newPoints, newOptions = {}) => updateGraphic('polylines', id, (polyline) => {
...
const addPolygon = (options = {}) => addGraphic('polygons', options, createPolygon);
const addPolygons = (polygonsList) => addGraphics('polygons', polygonsList, createPolygon);
const removePolygon = (id) => removeGraphic('polygons', id);
const clearPolygons = () => clearGraphics('polygons');
const updatePolygon = (id, newPoints, newOptions = {}) => updateGraphic('polygons', id, (polygon) => {
...
const addCircle = (options = {}) => addGraphic('circles', options, createCircle);
const addCircles = (circlesList) => addGraphics('circles', circlesList, createCircle);
const removeCircle = (id) => removeGraphic('circles', id);
const clearCircles = () => clearGraphics('circles');
const updateCircle = (id, newCircle, newOptions = {}) => updateGraphic('circles', id, (circle) => {
    if (newCircle) {
        const bCenter = createPoint(newCircle.center);
        if (bCenter) circle.setCenter(bCenter);
    }
    if (typeof newCircle.radius === 'number') circle.setRadius(newCircle.radius);
...
/**
 * 创建标记点对象
 * @param {Object} markerData 标记点数据对象
 ...
 * @returns {Object | null} 创建的标记点对象，失败返回null
 */
const createMarker = (markerData) => {
    if (!BMap) return null; // 检查 BMap
    return createGraphic('marker', markerData, (data) => {
        const point = createPoint(data.position);
        ...
        if (data.icon) {
            ...
            if (Array.isArray(anchor) && anchor.length === 2) {
                option.anchor = new BMap.Size(anchor[ 0 ], anchor[ 1 ]);
            }
            if (Array.isArray(imageOffset) && imageOffset.length === 2) {
                option.imageOffset = new BMap.Size(imageOffset[ 0 ], imageOffset[ 1 ]);
            }

            const icon = new BMap.Icon(url, new BMap.Size(size[ 0 ], size[ 1 ]), option);
...
/**
 * 创建折线对象
 * @param {Object} polylineData 折线数据对象
 ...
 * @returns {Object | null} 创建的折线对象，失败返回null
 */
const createPolyline = (polylineData) => {
    if (!BMap) return null; // 检查 BMap
    return createGraphic('polyline', polylineData, (data) => {
        const bPoints = data.points.map(createPoint).filter(bp => bp !== null);
        if (bPoints.length < 2) { // 折线至少需要两个点
            logger.warn(`创建折线失败，有效点不足: ${ bPoints.length }`, data);
            return null;
        }
        return new BMap.Polyline(bPoints, {
...
/**
 * 创建多边形对象
 * @param {Object} polygonData 多边形数据对象
 ...
 * @returns {Object | null} 创建的多边形对象，失败返回null
 */
const createPolygon = (polygonData) => {
    if (!BMap) return null; // 检查 BMap
    return createGraphic('polygon', polygonData, (data) => {
        const bPoints = data.points.map(createPoint).filter(bp => bp !== null);
        if (bPoints.length < 3) { // 多边形至少需要三个点
            logger.warn(`创建多边形失败，有效点不足: ${ bPoints.length }`, data);
            return null;
        }
        return new BMap.Polygon(bPoints, {
            strokeColor: data.strokeColor,
...
/**
 * 创建圆形对象
 * @param {Object} circleData 圆形数据对象
 ...
 * @returns {Object | null} 创建的圆形对象，失败返回null
 */
const createCircle = (circleData) => {
    if (!BMap) return null; // 检查 BMap
    return createGraphic('circle', circleData, (data) => {
        const center = createPoint(data.center);
        if (!center) {
            logger.warn('创建圆形失败，中心点无效', data);
            return null;
        }
        return new BMap.Circle(center, data.radius, {
            strokeColor: data.strokeColor,
            fillColor: data.fillColor,
            strokeWeight: data.strokeWeight,
            strokeOpacity: data.strokeOpacity,
...
/**
 * 创建自定义瓦片图层
 * @param {string} url 瓦片URL模板，例如 '/custom-tiles/{z}/{x}/{y}.png'
 * @returns {Object|null} 瓦片图层实例，创建失败返回null
 */
const createCustomTileLayer = (url) => {
    if (!window.BMap) {
 ...
 * @param {boolean} [visible=true] 是否可见，默认为 true
 * @returns {boolean} 操作是否成功
 */
const addTileLayer = (id, url, visible = true) => {
    if (!map) return false;
...
class TrajectoryAnimation {
     ...
     * @param {Array<Array<number>>} points 轨迹点坐标数组 [[经度, 纬度], ...]
     * @param {Object} [options] 动画选项
     * @param {number} [options.duration=5000] 动画持续时间(毫秒)
     * @param {boolean} [options.loop=false] 是否循环播放
     * @param {string} [options.icon] 移动图标URL
     * @param {Array<number>} [options.iconSize] 图标大小 [宽度, 高度]
     */
        ...
        try {
            ...
            if (this.options.icon) {
                const icon = new BMap.Icon(
                    ...
                    new BMap.Size(...(this.options.icon.size || [ 32, 32 ])),
                    {
                        anchor: new BMap.Size(...(this.options.icon.anchor || [ 16, 16 ])),
                        imageOffset: this.options.icon.imageOffset ? new BMap.Size(...this.options.icon.imageOffset) : new BMap.Size(0, 0)
                    }
                );
                this.marker = new BMap.Marker(startPoint, { icon });
            } else {
            ...
            if (this.options.startIcon) {
                const startIcon = new BMap.Icon(
                    ...
                    new BMap.Size(...(this.options.startIcon.size || [ 32, 32 ])),
                    {
                        anchor: new BMap.Size(...(this.options.startIcon.anchor || [ 16, 16 ])),
                        imageOffset: this.options.startIcon.imageOffset ? new BMap.Size(...this.options.startIcon.imageOffset) : new BMap.Size(0, 0)
                    }
                );
                this.startMarker = new BMap.Marker(startPoint, { icon: startIcon });
                // 设置起点标记的zIndex为较低层级
            ...
            if (this.options.endIcon) {
                ...
                const endIcon = new BMap.Icon(
                    ...
                    new BMap.Size(...(this.options.endIcon.size || [ 32, 32 ])),
                    {
                        anchor: new BMap.Size(...(this.options.endIcon.anchor || [ 16, 16 ])),
                        imageOffset: this.options.endIcon.imageOffset ? new BMap.Size(...this.options.endIcon.imageOffset) : new BMap.Size(0, 0)
                    }
                );
                this.endMarker = new BMap.Marker(endPoint, { icon: endIcon });
                // 设置终点标记的zIndex为较低层级
 ...
 */
const addTrajectory = (id, points, options = {}) => {
 ...
 */
const getTrajectory = (id) => instances.trajectories.get(id);
 ...
 */
const toggleGraphics = (type, id, visible = true) => {
    if (!map) return;
    ...
    try {
        ...
        if (!type) {
            // 处理所有标记点
            instances.markers.forEach(marker => {
                visible ? marker.show() : marker.hide();
            });
            // 处理所有折线
            instances.polylines.forEach(polyline => {
                visible ? polyline.show() : polyline.hide();
            });
            // 处理所有多边形
            instances.polygons.forEach(polygon => {
        ...
        // 控制单个图形
        const findGraphic = (collection, graphicId) => {
            return collection.find(item => item.userData && item.userData.id === graphicId);
        };
 ...
 */
const autoRecoverMap = async (forceReinit = false) => {
...
const checkEnvironment = () => {
    try {
        ...
        const result = {
            ...
            hasContainer,
            apiStatus,
            userAgent: hasBrowser ? window.navigator.userAgent : '未知'
        };
...
const emergencyDisconnect = () => {
    ...
    try {
        ...
        try {
            // 清理已知的全局回调
            for (const key in window) {
                if (typeof key === 'string' && (key.startsWith('BMap') || key.startsWith('initBaiduMap') || key.startsWith('_BMap'))) {
 ...
 */
const diffAndAddUpdateRemove = (type, oldList, newList, creator, updater, remover) => {
    if (!map) return;
    ...
    newList.forEach(newItem => {
        if (!oldMap.has(newItem.id)) {
            ...
            addGraphic(type, newItem, creator);
        } else {
            // 检查是否需要更新
